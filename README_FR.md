...
│   │
│   ├── build_json.php                → Génération du JSON d’onboarding client (FR)
│   │                                 → Client onboarding JSON builder (EN)
│   │
...
│   │
│   ├── onboarding/                   → Fichiers JSON issus du formulaire d’onboarding client (FR)
│   │                                 → JSON files generated by the client onboarding form (EN)
│   │ 
│   ├── onboarding_done/              → JSON d’onboarding traités et archivés après build (FR)
│   │                                 → Processed onboarding JSON archived after build (EN)
│   ├── archive_batch/
...
│   │
│   ├── find_client.php               → Recherche d’une fiche client (FR)
│   │                                 → Client lookup utility (EN)
│   │
│   ├── build_client.php              → Génération de l’index email (FR)
│   │                                 → Client email index builder (EN)
│   │ 
